...
fetch(apiURL, { headers })
  ...
  .then(data => {
   ...
   const jobValues = Object.values(data)
   
   HtmlContent = ''
   
   jobValues.forEach(item => {
        ...
        <tr>
          <td class="px-6 py-4 whitespace-no-wrap">
            <div class="flex items-center">
              <div class="ml-4">
              <div class="text-sm leading-5 font-medium text-gray-900">${item.jobTitle}</div>
              </div>
            </div>
            </td>
            <td class="px-6 py-4 whitespace-no-wrap">
              <div class="text-sm leading-5 text-gray-900">${item.minSalary}</div>
            </td>
            <td class="px-6 py-4 whitespace-no-wrap">
              <div class="text-sm leading-5 text-gray-900">${item.maxSalary}</div>
            </td>
